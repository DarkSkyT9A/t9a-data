...
(async () => {
  try {
    ...
    for (let tournament of tournamentsData) {
      ...
      console.log(`# of Tournament reports for ${tournament.name}: ${reports.length}`);
...
function transformReport(report) {
  ...
  // Not using T9A default scoring methodology
  if(report?.scoring.id !== 11)
  {
    return undefined;
  }

  // First turn
  r.firstTurn = report.first_turn;
  // Type (what is type?)
  r.type = report.type;
  // ID
  ...
  if(r.scoreOne + r.ScoreTwo !== 20) {
    console.error("### Faulty report, VP do not align to 20 ###");
  }

  r.armyOne = getArmyStringForId(r.players[0].id_book, true);
  r.armyTwo = getArmyStringForId(r.players[1].id_book, true);
